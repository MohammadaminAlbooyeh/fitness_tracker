...
async def create_behavior(behavior: schemas.UserBehaviorCreate, db: AsyncSession = Depends(get_db)):
    return await crud.create_behavior(db, behavior)


@app.get("/behaviors/{user_id}", response_model=list[schemas.UserBehavior])
async def get_behaviors(user_id: int, db: AsyncSession = Depends(get_db)):
